...
import org.finra.scaffolding.random.core.RubberRandom.{MaxRecursionCountName, RecursionEnabledName, RegexStrategyName}
import org.finra.scaffolding.random.core._
import org.finra.scaffolding.random.exceptions.RecursionDisabledException
import org.finra.scaffolding.random.predicate._
import org.finra.scaffolding.random.randomizers.LongRandomizer
import org.finra.scaffolding.random.support.AnnotationGenerator
import org.finra.scaffolding.random.types.{CollectionTypes, ParameterizedTypeReference, TypeContainer}
...
@ComponentScan(Array("org.finra.scaffolding.random.*"))
...
class RandomScalaTest extends TestCase {
    ...
    def testCustomRandomizerScan: Unit = {
        implicit var r = new RubberRandomImpl(
            Seq("org.finra.scaffolding", "com.dovidkopel")
        )
...
//        logger.debug("SSN: {}", ssn1)
    ...
    def test1(): Unit = {
        ...
        val bl = r.generate(classOf[org.finra.scaffolding.random.Food])
        logger.debug(mapper.writeValueAsString(bl))

        val ap = r.generate(new ParameterizedTypeReference[org.finra.scaffolding.random.Blah[Buffet[java.util.UUID]]]() {})
