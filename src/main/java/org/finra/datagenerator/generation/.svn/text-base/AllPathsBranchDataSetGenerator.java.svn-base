...
public class AllPathsBranchDataSetGenerator extends AbstractBranchDataSetGenerator {
	...
	private static Logger LOG = Logger.getLogger(AllPathsBranchDataSetGenerator.class);
	...
	protected List<DiskList<BranchGraphEdge>> allPaths(
		...
		List<DiskList<BranchGraphEdge>> pathsList = new LinkedList<DiskList<BranchGraphEdge>>();
		...
		for (BranchGraphEdge startEdge : graph.outgoingEdgesOf(rootNode)) {
			...
			Map<BranchGraphNode, Integer> nodeVisits = new HashMap<BranchGraphNode, Integer>();
			for (BranchGraphNode node : graph.vertexSet())
				nodeVisits.put(node, 0);
			// we start off visiting this edge and node
			LinkedList<BranchGraphEdge> curPath = new LinkedList<BranchGraphEdge>();
	...
	private void findPaths(
		...
		if (neighborEdges.size() == 0) // special case where we start on an isolated node
			pathsList.add(new DiskList<BranchGraphEdge>(curPath));
