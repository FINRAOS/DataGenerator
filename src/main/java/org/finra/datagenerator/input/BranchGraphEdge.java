...
public class BranchGraphEdge extends DefaultWeightedEdge implements IBranchGraphElement {
	...
	public BranchGraphEdge copy() {
		...
		try {
			for(CreateVariableRequirement val:createVarReqs) ret.addCreateVariableReq(val.clone());
			for(CreateGroupRequirement val:createGroupReqs) ret.addCreateGroupReq(val.clone());
			for(SetPropertyRequirement val:setPropReqs) ret.addSetPropertyReq(val.clone());
			for(CheckPropertyRequirement val:checkPropReqs) ret.addCheckPropertyReq(val.clone());
			for(AppendPropertyRequirement val:appendPropReqs) ret.addAppendPropertyReq(val.clone());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return ret;
